...
public:
  ...
  qtGradient();
  qtGradient(const QList<Stop>& stops,
  ...
  void setStops(const QList<Stop>&, NormalizeMode = NormalizeStops);
  bool insertStop(Stop stop);
  bool removeStop(qreal position);

  bool insertStop(qreal position, QColor color, qreal weight = 0.5)
    { return this->insertStop(qtGradient::Stop(position, color, weight)); }
